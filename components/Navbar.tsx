...
export default function Navbar() {
  ...
  return (
    ...
    >
      <div className="relative h-[150px] flex items-center justify-end px-6 md:px-10 overflow-hidden">
        <img
          src="/logo-navbar.jpg"
        ...
        <div className="relative z-10 hidden md:flex items-center gap-8 text-xs uppercase tracking-widest text-gray-800">
          ...
          <a href="#reserva" className="bg-[#4a6741] text-white px-5 py-2 rounded-md hover:bg-[#6b8f62] transition">Reservar</a>
        </div>

        <button onClick={() => setOpen(!open)} className="relative z-10 md:hidden text-2xl">
          ☰
        </button>
      </div>
      ...
      {open && (
        <div className="md:hidden bg-white border-t border-[#e0ddd6] p-6 flex flex-col gap-4 text-sm shadow-md">
          ...
          <a href="#reserva" onClick={() => setOpen(false)} className="bg-[#4a6741] text-white px-4 py-2 rounded text-center">Reservar</a>
        </div>
      )}
    </nav>
  );
